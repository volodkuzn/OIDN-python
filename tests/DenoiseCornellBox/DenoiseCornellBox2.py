...
with oidn.Device("cpu") as device, oidn.Filter(device, "RT") as filter:
    input = oidn.Buffer.load(
        device, Image.open((here / "CornellBoxNoisy.png").as_posix()), normalize=True
    )
    ...
    filter.execute()
    Image.fromarray(np.array(np.clip(output.to_array() * 255, 0, 255), dtype=np.uint8)).save(
        f"{here}/CornellBoxDenoised.png"
    )
