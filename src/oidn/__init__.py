...
def __load_lib():
    ...
    if sys.platform == "linux":
        ctypes.CDLL(
            os.path.join(cur_path, f"lib.linux.x64/libOpenImageDenoise_core.so.{oidn_version}")
        )
        ctypes.CDLL(
            os.path.join(
                cur_path, f"lib.linux.x64/libOpenImageDenoise_device_cpu.so.{oidn_version}"
            )
        )
        return ctypes.CDLL(
            os.path.join(cur_path, f"lib.linux.x64/libOpenImageDenoise.so.{oidn_version}")
        )
        ...
        if "x86_64" in out or "x64" in out:
            ...
            raise RuntimeError(
                "Unknown architecture of this machine, candidate architectures are x64 and aarch64"
            )
        if arch == "aarch64":
            ctypes.CDLL(
                os.path.join(
                    cur_path, f"lib.macos.aarch64/libOpenImageDenoise_core.{oidn_version}.dylib"
                )
            )
            ctypes.CDLL(
                os.path.join(
                    cur_path,
                    f"lib.macos.aarch64/libOpenImageDenoise_device_cpu.{oidn_version}.dylib",
                )
            )
            return ctypes.CDLL(
                os.path.join(
                    cur_path, f"lib.macos.aarch64/libOpenImageDenoise.{oidn_version}.dylib"
                )
            )
        else:  # x64
            ctypes.CDLL(
                os.path.join(
                    cur_path, f"lib.macos.x64/libOpenImageDenoise_core.{oidn_version}.dylib"
                )
            )
            ctypes.CDLL(
                os.path.join(
                    cur_path, f"lib.macos.x64/libOpenImageDenoise_device_cpu.{oidn_version}.dylib"
                )
            )
            return ctypes.CDLL(
                os.path.join(cur_path, f"lib.macos.x64/libOpenImageDenoise.{oidn_version}.dylib")
            )
    elif sys.platform == "win32":
        ctypes.CDLL(os.path.join(cur_path, "lib.win.x64/OpenImageDenoise_core.dll"))
        ctypes.CDLL(os.path.join(cur_path, "lib.win.x64/OpenImageDenoise_device_cpu.dll"))
        return ctypes.CDLL(os.path.join(cur_path, "lib.win.x64/OpenImageDenoise.dll"))
    else:
